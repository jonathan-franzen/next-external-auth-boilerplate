...
async function AdminPage({ searchParams }: AdminPageProps): Promise<ReactNode> {
	...
	const { data, error } = await until(() => getUsersApiAction({ page: pageNumber, sortBy }, true));
	...
	return (
		<>
			...
			{data && data.pagination.totalPages > 1 && <PageSelector className='mt-4' currentPage={data.pagination.page} totalPages={data.pagination.totalPages} />}
		</>
	);
}
