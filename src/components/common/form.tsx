...
function Form({
  ...
  const handleOnSubmit = async (
    ...
    const errorMessage = Object.values(newErrors).find(
    ...
    >
        ...
        (field: FieldReactFormInterface): ReactNode => (
          <div className="mt-2" key={field.name}>
            ...
            {formErrors[field.name] &&
              validationSchema &&
              validationSchema[field.name]?.showError && (
                <div className="mt-1 text-xs text-pink-900">
                  {formErrors[field.name]}
                </div>
              )}
          </div>
      ...
      {errorMessage && (
        <div className="relative mt-2 flex items-center justify-center gap-3 rounded-md bg-pink-50 p-2">
          ...
          <button
            className="absolute right-4 text-pink-900"
            onClick={() => void setErrorMessage(undefined)}
          >
            <CircleX fill="#831843" name="close-circle" size="16" />
          </button>
        </div>
      )}
    </form>
