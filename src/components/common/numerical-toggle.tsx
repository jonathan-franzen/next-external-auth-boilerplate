...
function NumericalToggle({
  className,
  currentValue,
  onValueChange,
  renderValue,
  totalValues,
}: NumericalToggleProps): ReactNode {
  ...
  const getDisplayedValues = (): (number | string)[] => {
    ...
    for (
      let i = Math.max(2, currentValue - 1);
      i <= Math.min(totalValues - 1, currentValue + 1);
      i++
    ) {
      values.push(i)
    }
      ...
      {displayedValues.map((value, index) =>
        typeof value === 'number' ? (
          <GhostButton
            className={
              value === currentValue
                ? 'cursor-default font-bold hover:text-pink-900'
                : ''
            }
            key={index}
